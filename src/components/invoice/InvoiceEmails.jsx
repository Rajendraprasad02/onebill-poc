...
const InvoiceEmails = () => {
  ...
  return (
    <div className="p-6">
      <div className="relative" ref={menuRef}>
        ...
        {menuOpen && (
          <div className="absolute right-0 mt-2 w-52 bg-white shadow-lg rounded-md p-2">
            <button
              onClick={handleLogout}
              className="w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 rounded-md cursor-pointer"
            >
              Logout
            </button>
          </div>
      ...
      <ul className="space-y-4 w-full flex justify-center flex-col">
        {emails.map((email, index) => (
          <li key={email.id} className="bg-gray-50 rounded-lg shadow-md p-4">
            <h3 className="font-semibold text-xl">{email.from}</h3>
